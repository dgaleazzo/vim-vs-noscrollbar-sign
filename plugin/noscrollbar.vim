...
function! noscrollbar#statusline(...)
    ...
    redir => signs
    silent execute 'sign place buffer='.current_buffer_number
    redir END
    let signs = split(signs, '\n')
    let sign_lines = []
    for s in signs[2:]
      let l = split(split(s, '=')[1], " ")[0]
      call add(sign_lines, l)
    endfor
...
\       && (gripper_position + gripper_length == scrollbar_length)
    ...
    for sign_line in sign_lines
      let l = float2nr(round(1.0 * sign_line / lines_count * scrollbar_length))
      let scrollbar = strpart(scrollbar, 0, l-1) . signsymbol . strpart(scrollbar, l)
    endfor
